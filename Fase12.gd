...
func mostrarQtd(a,b):
	var tipo=randi()%2+1 #sortear numero ou bixo
	if tipo==1:#numero
		for i in range(1,10):
			if i==a:#lado a
				get_node("/root/"+ get_tree().get_current_scene().get_name() +"/AnimatedSpriteA"+str(i)).hide()
				get_node("/root/"+ get_tree().get_current_scene().get_name() +"/SpriteA"+str(i)).hide()
				get_node("/root/"+ get_tree().get_current_scene().get_name() +"/NumeroA"+str(i)).show()
			else:
				get_node("/root/"+ get_tree().get_current_scene().get_name() +"/AnimatedSpriteA"+str(i)).hide()
				get_node("/root/"+ get_tree().get_current_scene().get_name() +"/SpriteA"+str(i)).hide()
				get_node("/root/"+ get_tree().get_current_scene().get_name() +"/NumeroA"+str(i)).hide()
					
	else:#animais
		for i in range(1,10):
			var subTipo=randi()%2+1#gato ou cachorro
			if subTipo==1:
				if i<=a:#lado a
					get_node("/root/"+ get_tree().get_current_scene().get_name() +"/AnimatedSpriteA"+str(i)).show()
					get_node("/root/"+ get_tree().get_current_scene().get_name() +"/SpriteA"+str(i)).hide()
					get_node("/root/"+ get_tree().get_current_scene().get_name() +"/NumeroA"+str(i)).hide()
				else:
					get_node("/root/"+ get_tree().get_current_scene().get_name() +"/AnimatedSpriteA"+str(i)).hide()
	...
	if tipo==1:#numero
		for i in range(1,10):
			if i==b:#lado b
				get_node("/root/"+ get_tree().get_current_scene().get_name() +"/AnimatedSpriteB"+str(i)).hide()
				get_node("/root/"+ get_tree().get_current_scene().get_name() +"/SpriteB"+str(i)).hide()
				get_node("/root/"+ get_tree().get_current_scene().get_name() +"/NumeroB"+str(i)).show()
			else:
				get_node("/root/"+ get_tree().get_current_scene().get_name() +"/AnimatedSpriteB"+str(i)).hide()
				get_node("/root/"+ get_tree().get_current_scene().get_name() +"/SpriteB"+str(i)).hide()
				get_node("/root/"+ get_tree().get_current_scene().get_name() +"/NumeroB"+str(i)).hide()
		...
		for i in range(1,10):
			...
			if subTipo==1:
				if i<=b:#lado b
					get_node("/root/"+ get_tree().get_current_scene().get_name() +"/AnimatedSpriteB"+str(i)).show()
					get_node("/root/"+ get_tree().get_current_scene().get_name() +"/SpriteB"+str(i)).hide()
					get_node("/root/"+ get_tree().get_current_scene().get_name() +"/NumeroB"+str(i)).hide()
				else:
					get_node("/root/"+ get_tree().get_current_scene().get_name() +"/AnimatedSpriteB"+str(i)).hide()
